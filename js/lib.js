...
jQuery(document).ready(function(){
	...
	jQuery(".sc_contact_form .enter").click(function (e) {
		...
		empt = 'Поле "Имя" не может быть пустым';
     to_lng = 'Слишком длинное поле имени';
     to_lng = 'Слишком длинное поле имени';
     empt_mail = 'Слишком короткий (или пустой) адрес электронной почты';
     to_lng_mail = 'Слишком длинный адрес электронной почты';
     incor = 'Неверный адрес электронной почты';
     mes_empt = 'Сообщение не может быть пустым';
     to_lng_mes = 'Слишком длинное сообщение';
...
/*
			jQuery.post("comment.php", data, userSubmitFormResponse, "text");
			setTimeout(function(){
				jQuery.ajax({
					method: "POST",
					url: "getcomment.php"
